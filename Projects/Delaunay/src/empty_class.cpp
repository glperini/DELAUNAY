...
///DEF MergeSorter
//Algoritmo MergeSort "rivisitato"
vector<Point> MergeSorter::MergeSort(vector<Point>& v,
                                     int& sx,
                                     int& dx,
                                     bool& sortingByX)
{
    int cx;
    vector<Point> sortedVector;
    sortedVector.reserve(v.size());

    if (sx<dx)
    {
        cx = (sx + dx)/2;
        MergeSort(v, sx, cx, sortingByX);
        int cx_piu1 = cx + 1;
        MergeSort(v, cx_piu1, dx, sortingByX);
        Merge(v, sx, cx, dx, sortingByX);
        sortedVector = v;
    }
...
void MergeSorter::Merge(vector<Point>& v,                //a me serve che mergesort ordini degli indici cosi mmi tengo entrambe le coordinate
                        int& sx,
                        int& cx,
                        int& dx,
                        bool& sortingByX)
{
    ...
    while ((i<=cx) && (j<=dx))
    {
        if (getOrder(v[i], v[j], sortingByX).x==v[i].x && getOrder(v[i], v[j], sortingByX).y==v[i].y) {
            b[k] = v[i];
            i++;
        }
        else {
    ...
    for (; i<=cx; i++, k++)     //viene eseguito se ancora ci sono elementi rimanenti nel primo sottovettore (da i a cx)
        b[k] = v[i];
    for (; j<=dx; j++, k++)     //viene eseguito se ancora ci sono elementi rimanenti nel secondo sottovettore (da j a dx)
        b[k] = v[j];
    for (i=sx; i<=dx; i++)
        v[i] = b[i-sx];
}
...
Point MergeSorter::getOrder(Point& point1, Point& point2, bool& sortingByX){
    if (sortingByX == true) {
...
Vector2d Segment::lineCoefficients(Point& p1, Point& p2){       //ATTENZIONE: trattare caso retta verticale (se hanno la stessa x)
...
//Verfica se il punto si trova su un segmento già esistente
//Se sì, lo collego con i due vertici dei triangoli adiacenti
/*void Delaunay::isPointOnSegment(Point& point, vector<Triangle>& triangles) {
    Segment temporary;
    Vector2d line = temporary.lineCoefficients()
...
//unisce punto interno con 3 vertici x creare sottotriangolazione
 void Delaunay::inTriangle(Point& point, vector<Triangle>& triangles, int& triangleIndex, vector<int>& hullTrianglesIndices){
    ...
    /*//aggiorniamo le adiacenze del triangolo vicino a t1
    for (int i=0; i<3; i++) {
        if (t1.neighbors[0] != -1) {
            if (triangles[t1.neighbors[0]].neighbors[i] == triangleIndex) {
                triangles[t1.neighbors[0]].neighbors[i] = triangleIndex;
                break;
            }
        }
    }*/
